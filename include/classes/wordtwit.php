	...
	function construct_admin_menu() {
		if ( function_exists( 'add_meta_box' ) ) {
			...
			foreach ( $post_types as $post_type ) {
				add_meta_box( 'wordtwit-box', __( 'WordTwit', 'wordtwit-pro' ), array(
					&$this,
					'show_post_box',
				), $post_type, 'side' );
			}
				...
				case 'update-account-order':
					if ( isset( $this->post['account_order'] ) ) {
						...
						if ( $account_order ) {
							...
							delete_user_meta( $cur_user->ID, 'wordtwit_account_order' );
							add_user_meta( $cur_user->ID, 'wordtwit_account_order', $account_order );
						}
					}
					break;
				default:
				...
				switch ( $wordtwit_account_action ) {
					case 'delete':
						...
						if ( wordtwit_user_is_admin() ) {
							...
							if ( $post_info->post_author == $cur_user->ID ) {
								$can_delete = true;
							}
						}
